...
export class PlannerAgent extends BaseAgent<PlannerInput, PlannerOutput> {
  ...
  constructor() {
    ...
    // Initialize RAG service after super()
  ...
  async run(input: PlannerInput): Promise<PlannerOutput> {
    ...
    if (this.ragService.isInitialized()) {
      try {
        ...
        const ragQuery = `${input.issueTitle}\n\n${input.issueBody}`;
        const ragResults = await this.ragService.search(ragQuery);
        
        if (ragResults.snippets) {
          ragContext += `\n\n## Relevant Code Snippets from RAG Search\n${ragResults.snippets}\n`;
        }
        if (ragResults.suggestedFiles && ragResults.suggestedFiles.length > 0) {
          ragContext += `\n## Suggested Files from RAG\n${ragResults.suggestedFiles.map(f => `- ${f}`).join('\n')}\n`;
        }
      } catch (error) {
        console.error("[Planner] RAG search failed:", error);
        // Continue without RAG context on error
      }
    }

    // Enrich issue body with RAG context
